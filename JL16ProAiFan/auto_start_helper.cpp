...
#include "pch.h"  // 包含 MFC 的基本类和功能，包括 AfxMessageBox 和 CString
//#include <windows.h>
//#include <iostream>
#include <stdio.h>
#include <comdef.h>
...
#include <taskschd.h>
#include <wchar.h>  // 引入必要的头文件_wdupenv_s
...
bool create_auto_start_task_for_this_user(bool runElevated)
{
    ...
    // Task Name.
    wstrTaskName = L"Autorun JL16ProAiFan for ";
    ...
    // ------------------------------------------------------
    // Get the JL16ProAiFan task folder. Creates it if it doesn't exist.
    hr = pService->GetFolder(_bstr_t(L"\\JL16ProAiFan"), &pTaskFolder);
    if (FAILED(hr))
    {
        // Folder doesn't exist. Get the Root folder and create the JL16ProAiFan subfolder.
        ITaskFolder* pRootFolder = NULL;
        hr = pService->GetFolder(_bstr_t(L"\\"), &pRootFolder);
        ExitOnFailure(hr, "Cannot get Root Folder pointer: %x", hr);
        hr = pRootFolder->CreateFolder(_bstr_t(L"\\JL16ProAiFan"), _variant_t(L""), &pTaskFolder);
        if (FAILED(hr))
        {
            pRootFolder->Release();
            ExitOnFailure(hr, "Cannot create JL16ProAiFan task folder: %x", hr);
        }
    }
    ...
    // Add the logon trigger to the task.
    {
        ...
        hr = pLogonTrigger->put_Delay(_bstr_t(L"PT07S"));
...
bool delete_auto_start_task_for_this_user()
{
    ...
    // ------------------------------------------------------
    // Get the JL16ProAiFan task folder.
    hr = pService->GetFolder(_bstr_t(L"\\JL16ProAiFan"), &pTaskFolder);
    if (FAILED(hr))
    {
