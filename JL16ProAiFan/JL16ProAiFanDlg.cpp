	...
	//if (isJL16Pro()) {
		{
			...
			if (foundPosBIOSVersion != std::wstring::npos) {
				CFanControl::BIOSVersionNoV31 = false;
			}
			else
			{
			...
			if (_stscanf_s(szValue, _T("%d, %d, %d"), &r, &g, &b) != 3)
			{
				...
				r = CFanControl::FCEC.readByte(KeyLightR);
				g = CFanControl::FCEC.readByte(KeyLightG);
				b = CFanControl::FCEC.readByte(KeyLightB);
			}
			...
			else
			{
				...
				if (MaxFanSpeedSet >= FanSpeedSetLow && MaxFanSpeedSet <= FanSpeedSetHigh  || MaxFanSpeedSet == 0)
				{
			...
			m_SLIDER_MaxFanSpeedSet.SetRange(FanSpeedSetLow - 1, FanSpeedSetHigh);//设置范围
...
void CJL16ProAiFanDlg::OnTimer(UINT_PTR nIDEvent)
{
	switch (nIDEvent)
	{
		case TStartAiFanControl: // 假设IDT_TIMER1是你为某个特定任务设置的定时器ID
		{
			...
			else if (CFanControl::BIOSVersionNoV31 && !CFanControl::m_FanSetStatus )
			{
				//高温恢复风扇调速控制
...
void CJL16ProAiFanDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	case IDC_SLIDER_MaxFanSpeedSet:
		...
		{
			UpdateData(TRUE);
			//CFanControl::m_MaxFanSpeedSet
			BYTE FanSpeedGet = m_SLIDER_MaxFanSpeedSet.GetPos();
			UpdateData(FALSE);

			if (FanSpeedGet < FanSpeedSetLow)
			{
				CFanControl::m_MaxFanSpeedSet = 0;
			}
			else
